...
## Katechon-specific rules

- Read `../../docs/katechon-company-context.md` before authoring investor-facing slides.
- Use `themes/katechon-generative.md` as the default aesthetic and content guardrail.
- Treat `examples/` as reference-only code. Do not put example decks under `slides/`.
- The live generated target is `slides/live-generated/index.tsx`; realtime generation may overwrite that file.
- Each generated page should feel unique, but it should still reuse the Katechon background/template grammar instead of inventing a new brand system.
- Never invent traction, revenue, customer names, live market data, signed deals, valuations, or regulated execution claims.
- Prefer precise language around channels, state, surfaces, agents, provenance, sharing, forking, and the post-page internet.
