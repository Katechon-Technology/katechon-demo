    ...
    function renderStage() {
      ...
      runSceneMotion();
      if (dashboardId === "iran") {
        installIranSlideDelegation();
        requestAnimationFrame(renderIranActiveSlideMap);
      }
      requestAnimationFrame(renderGeneratedCharts);
    }
    ...
    function renderBlankStage() {
      ...
      if (!stage) return;
      if (dashboardId === "iran") {
        stage.innerHTML = renderIranSceneWrapper();
        installIranSlideDelegation();
        requestAnimationFrame(renderIranActiveSlideMap);
        return;
      }
    ...
    function renderIranSceneWrapper() {
      const slide = state.iranSlide || 0;
      const content = slide === 0 ? renderIranHormuzScene() : renderIranEnergyScene();
      return `<div class="iran-stage" data-slide="${slide}">
        ${content}
        <nav class="iran-slide-nav" aria-label="Iran dashboard slide navigation">
          <button type="button" class="iran-slide-btn" data-iran-slide-action="prev" aria-label="Previous slide"${slide === 0 ? " disabled" : ""}>&#x2039;</button>
          <span class="iran-slide-indicator" aria-hidden="true">
            <span class="iran-slide-dot${slide === 0 ? " is-active" : ""}" data-iran-slide-action="set" data-slide="0"></span>
            <span class="iran-slide-dot${slide === 1 ? " is-active" : ""}" data-iran-slide-action="set" data-slide="1"></span>
          </span>
          <span class="iran-slide-counter">${slide === 0 ? "Routes" : "Energy"} &middot; ${slide + 1}/2</span>
          <button type="button" class="iran-slide-btn" data-iran-slide-action="next" aria-label="Next slide"${slide === 1 ? " disabled" : ""}>&#x203A;</button>
        </nav>
      </div>`;
    }

    function renderIranActiveSlideMap() {
      if ((state.iranSlide || 0) === 0) renderHormuzVegaMap();
      else renderHormuzEnergyMap();
    }

    let iranSlideDelegationInstalled = false;
    function installIranSlideDelegation() {
      if (iranSlideDelegationInstalled) return;
      iranSlideDelegationInstalled = true;
      document.body.addEventListener("click", (event) => {
        const trigger = event.target.closest && event.target.closest("[data-iran-slide-action]");
        if (!trigger) return;
        if (dashboardId !== "iran") return;
        if (trigger.hasAttribute("disabled")) return;
        const action = trigger.dataset.iranSlideAction;
        const current = state.iranSlide || 0;
        if (action === "prev") setIranSlide(current - 1);
        else if (action === "next") setIranSlide(current + 1);
        else if (action === "set") setIranSlide(Number(trigger.dataset.slide) || 0);
      });
      document.addEventListener("keydown", (event) => {
        if (dashboardId !== "iran") return;
        ...
        if (event.key === "ArrowRight") setIranSlide((state.iranSlide || 0) + 1);
        else if (event.key === "ArrowLeft") setIranSlide((state.iranSlide || 0) - 1);
      });
    }

    function setIranSlide(index) {
      const next = Math.max(0, Math.min(1, Number(index) || 0)) | 0;
      if ((state.iranSlide || 0) === next) return;
      state.iranSlide = next;
      const stage = $("stage");
      if (!stage) return;
      const existing = stage.querySelector(".iran-stage");
      if (existing) {
        const wrap = document.createElement("div");
        wrap.innerHTML = renderIranSceneWrapper();
      ...
      if (document.body.dataset.generatedView === "blank") renderBlankStage();
      else renderStage();
    }

    function renderIranHormuzScene() {
      return `<div class="hormuz-dashboard" data-testid="hormuz-dashboard">
        <section class="hormuz-map-shell" aria-label="Vega map of the Strait of Hormuz">
          <div class="hormuz-map-title">
            <span>Vega geospatial layer</span>
            <strong>Strait of Hormuz</strong>
            <em>Bandar Abbas, Qeshm, Musandam, and outbound shipping lanes</em>
          </div>
          <div class="hormuz-map" id="hormuz-vega-map" data-testid="hormuz-vega-map"></div>
        </section>
        <aside class="hormuz-context" aria-label="Hormuz map context">
          <div>
            <div class="hormuz-context-kicker">regional context</div>
            <h2>Measured route map for the Gulf chokepoint.</h2>
          </div>
          <div class="hormuz-stat-grid">
            <article class="hormuz-stat">
              <div class="hormuz-stat-label">narrowest</div>
              <strong>21 nm</strong>
              <span>Public reference width near the shipping corridor.</span>
            </article>
            <article class="hormuz-stat">
              <div class="hormuz-stat-label">lanes</div>
              <strong>2 x 2 mi</strong>
              <span>Inbound and outbound traffic separation lanes.</span>
            </article>
            <article class="hormuz-stat">
              <div class="hormuz-stat-label">nodes</div>
              <strong>6</strong>
              <span>Ports, islands, and approach markers in view.</span>
            </article>
          </div>
          <div class="hormuz-legend" aria-label="Map legend">
            <div class="hormuz-legend-row"><span class="hormuz-legend-line"></span><span>Shipping lanes and chokepoint transect</span></div>
            <div class="hormuz-legend-row"><span class="hormuz-legend-dot" style="background:var(--accent2)"></span><span>Ports and mainland infrastructure nodes</span></div>
            <div class="hormuz-legend-row"><span class="hormuz-legend-dot" style="background:var(--accent)"></span><span>Island and coastal reference points</span></div>
          </div>
        </aside>
      </div>`;
    }

    function hormuzMapData() {
      const land = [
        {
          type: "Feature",
          properties: { name: "Iran mainland", side: "iran" },
          geometry: {
            type: "Polygon",
            coordinates: [[
              [54.65, 27.65], [54.65, 26.56], [55.18, 26.47], [55.58, 26.58],
              [55.92, 26.73], [56.24, 26.96], [56.52, 27.16], [57.16, 27.05],
              [57.72, 26.86], [57.72, 27.65], [54.65, 27.65],
            ]],
          },
        },
        {
          type: "Feature",
          properties: { name: "Qeshm Island", side: "iran-island" },
          geometry: {
            type: "Polygon",
            coordinates: [[
              [55.22, 26.66], [55.46, 26.78], [55.92, 26.82], [56.12, 26.72],
              [55.92, 26.59], [55.48, 26.57], [55.22, 26.62], [55.22, 26.66],
            ]],
          },
        },
        {
          type: "Feature",
          properties: { name: "Musandam / Oman coast", side: "oman" },
          geometry: {
            type: "Polygon",
            coordinates: [[
              [54.65, 24.85], [57.72, 24.85], [57.72, 25.61], [57.16, 25.64],
              [56.74, 25.78], [56.38, 25.98], [56.18, 26.19], [55.88, 26.36],
              [55.42, 26.20], [55.02, 25.88], [54.65, 25.54], [54.65, 24.85],
            ]],
          },
        },
      ];
      const routes = [
        {
          type: "Feature",
          properties: { name: "Outbound lane", kind: "shipping" },
          geometry: { type: "LineString", coordinates: [[54.86, 26.22], [55.34, 26.25], [55.88, 26.24], [56.24, 26.18], [56.78, 25.98], [57.46, 25.69]] },
        },
        {
          type: "Feature",
          properties: { name: "Inbound lane", kind: "shipping" },
          geometry: { type: "LineString", coordinates: [[54.86, 26.05], [55.35, 26.08], [55.88, 26.08], [56.25, 26.03], [56.70, 25.86], [57.36, 25.58]] },
        },
        {
          type: "Feature",
          properties: { name: "Fujairah approach", kind: "approach" },
          geometry: { type: "LineString", coordinates: [[56.33, 25.14], [56.48, 25.46], [56.70, 25.86]] },
        },
      ];
      const chokepoint = [{
        type: "Feature",
        properties: { name: "Hormuz chokepoint transect" },
        geometry: { type: "LineString", coordinates: [[56.30, 26.77], [56.46, 26.13]] },
      }];
      const sites = [
        { label: "Bandar Abbas", lon: 56.27, lat: 27.18, type: "port", weight: 88, dx: 10, dy: -10 },
        { label: "Qeshm", lon: 55.90, lat: 26.81, type: "island", weight: 62, dx: -44, dy: -12 },
        { label: "Hormuz Island", lon: 56.46, lat: 27.06, type: "island", weight: 70, dx: 10, dy: -6 },
        { label: "Khasab", lon: 56.25, lat: 26.18, type: "port", weight: 62, dx: 10, dy: 13 },
        { label: "Fujairah", lon: 56.34, lat: 25.13, type: "port", weight: 56, dx: 10, dy: 10 },
        { label: "UAE approach", lon: 55.18, lat: 25.26, type: "watch", weight: 48, dx: 10, dy: 10 },
      ];
      const labels = [
        { label: "Persian Gulf approach", lon: 55.08, lat: 26.42, tone: "water", dx: 0, dy: 0, angle: -3 },
        { label: "Gulf of Oman", lon: 57.06, lat: 25.24, tone: "water", dx: 0, dy: 0, angle: -15 },
        { label: "Strait of Hormuz", lon: 56.50, lat: 26.43, tone: "choke", dx: 12, dy: -18, angle: -18 },
      ];
      return { land, routes, chokepoint, sites, labels };
    }

    function buildHormuzVegaSpec(width, height) {
      const accent = cssToken("--accent", "#00e87b");
      const accent2 = cssToken("--accent2", "#7de8ff");
      const accent3 = cssToken("--accent3", "#ffbf5f");
      const danger = cssToken("--danger", "#ff625f");
      const text = cssToken("--text", "#f3f6f8");
      const data = hormuzMapData();
      const projectionScale = Math.max(4200, Math.min(width / 0.058, height / 0.057));
      return {
        $schema: "https://vega.github.io/schema/vega/v6.json",
        description: "Strait of Hormuz map with ports, islands, shipping lanes, and chokepoint context.",
        background: "transparent",
        width,
        height,
        padding: 0,
        autosize: "none",
        signals: [
          { name: "pulse", value: 0, on: [{ events: { type: "timer", throttle: 80 }, update: "(pulse + 1) % 1000" }] },
        ],
        data: [
          { name: "graticule", transform: [{ type: "graticule", extent: [[54.5, 24.75], [57.8, 27.75]], step: [0.5, 0.5] }] },
          { name: "land", values: data.land },
          { name: "routes", values: data.routes },
          { name: "chokepoint", values: data.chokepoint },
          {
            name: "sites",
            values: data.sites,
            transform: [{ type: "geopoint", projection: "hormuz", fields: ["lon", "lat"], as: ["x", "y"] }],
          },
          {
            name: "labels",
            values: data.labels,
        ...
        scales: [
          { name: "siteColor", type: "ordinal", domain: ["port", "island", "watch"], range: [accent2, accent, accent3] },
          { name: "siteSize", type: "linear", domain: [45, 90], range: [70, 250] },
        ],
        marks: [
          {
            type: "rect",
          ...
          },
          {
            ...
            from: { data: "graticule" },
            interactive: false,
            transform: [{ type: "geoshape", projection: "hormuz" }],
            encode: {
              enter: {
                fill: { value: null },
                stroke: { value: "rgba(255,255,255,0.075)" },
                strokeWidth: { value: 1 },
              },
            },
          },
          {
            ...
            encode: {
              update: {
                fill: [
                  { test: "datum.properties.side === 'iran'", value: "rgba(255,191,95,0.28)" },
                  { test: "datum.properties.side === 'iran-island'", value: "rgba(0,232,123,0.26)" },
                  { value: "rgba(125,232,255,0.20)" },
                ],
                stroke: { value: "rgba(243,246,248,0.34)" },
                strokeWidth: { value: 1 },
              },
              hover: {
                fillOpacity: { value: 0.78 },
          ...
          },
          {
            ...
            encode: {
              update: {
                fill: { value: null },
                stroke: [
                  { test: "datum.properties.kind === 'approach'", value: accent2 },
                  { value: accent3 },
                ],
                strokeWidth: [
                  { test: "datum.properties.kind === 'approach'", value: 1.7 },
                  { value: 2.4 },
                ],
                strokeOpacity: { value: 0.86 },
                strokeDash: { value: [8, 7] },
                strokeDashOffset: { signal: "-pulse" },
              },
            },
          },
          {
            type: "shape",
            from: { data: "chokepoint" },
            transform: [{ type: "geoshape", projection: "hormuz" }],
            encode: {
              enter: {
                fill: { value: null },
          ...
          },
          {
            ...
            encode: {
              enter: {
                x: { field: "x" },
                y: { field: "y" },
                size: { scale: "siteSize", field: "weight" },
                fill: { scale: "siteColor", field: "type" },
                fillOpacity: { value: 0.88 },
                stroke: { value: "rgba(255,255,255,0.62)" },
              ...
              hover: {
                size: { signal: "scale('siteSize', datum.weight) * 1.35" },
                stroke: { value: text },
              },
            },
          },
          {
            ...
            encode: {
              update: {
                x: { field: "x" },
                y: { field: "y" },
                size: { signal: "scale('siteSize', datum.weight) * (1.6 + (pulse % 30) / 70)" },
                fill: { value: null },
                stroke: { scale: "siteColor", field: "type" },
                strokeOpacity: { signal: "0.42 - ((pulse % 30) / 90)" },
                strokeWidth: { value: 1 },
              },
            },
          },
          {
            ...
            encode: {
              enter: {
                ...
                dy: { field: "dy" },
                text: { field: "label" },
                fill: { value: "rgba(243,246,248,0.82)" },
                font: { value: "SFMono-Regular, Consolas, monospace" },
                fontSize: { value: 10 },
          ...
          },
          {
            ...
            encode: {
              enter: {
                ...
                baseline: { value: "middle" },
                fill: [
                  { test: "datum.tone === 'choke'", value: danger },
                  { value: "rgba(125,232,255,0.58)" },
                ],
                font: { value: "SFMono-Regular, Consolas, monospace" },
                fontSize: { value: 11 },
                fontWeight: { value: 760 },
              },
            },
          },
        ],
    ...
    function renderIranEnergyScene() {
      return `<div class="hormuz-dashboard hormuz-dashboard-energy" data-testid="hormuz-dashboard-energy">
        <section class="hormuz-map-shell hormuz-map-shell-energy" aria-label="Vega energy corridor map of the Persian Gulf">
          <div class="hormuz-map-title">
            <span>Vega energy layer</span>
            <strong>Persian Gulf Energy Corridor</strong>
            <em>Pipelines, oil terminals, tanker traffic, and chokepoint pressure rings</em>
          </div>
          <div class="hormuz-map hormuz-map-energy" id="hormuz-vega-map-2" data-testid="hormuz-vega-map-2"></div>
          <div class="hormuz-energy-callouts" aria-hidden="true">
            <span class="hormuz-energy-chip"><strong>17M</strong>bbl/day transit</span>
            <span class="hormuz-energy-chip"><strong>~20%</strong>of world oil</span>
            <span class="hormuz-energy-chip"><strong>~30%</strong>seaborne LNG</span>
          </div>
        </section>
        <aside class="hormuz-context" aria-label="Energy corridor context">
          <div>
            <div class="hormuz-context-kicker">strategic energy layer</div>
            <h2>Pipelines, terminals, and the Hormuz transit gauntlet.</h2>
          </div>
          <div class="hormuz-stat-grid">
            <article class="hormuz-stat">
              <div class="hormuz-stat-label">ADCOP bypass</div>
              <strong>1.5M bpd</strong>
              <span>Habshan to Fujairah, routes UAE crude around the chokepoint.</span>
            </article>
            <article class="hormuz-stat">
              <div class="hormuz-stat-label">Goreh - Jask</div>
              <strong>300K bpd</strong>
              <span>Iran's southern bypass line to the Gulf of Oman terminal.</span>
            </article>
            <article class="hormuz-stat">
              <div class="hormuz-stat-label">terminals in view</div>
              <strong>8</strong>
              <span>Major oil and LNG export terminals lining both shores.</span>
            </article>
          </div>
          <div class="hormuz-legend" aria-label="Energy map legend">
            <div class="hormuz-legend-row"><span class="hormuz-legend-line hormuz-legend-line-pipeline"></span><span>Oil pipelines (capacity, bbl/day)</span></div>
            <div class="hormuz-legend-row"><span class="hormuz-legend-dot hormuz-legend-dot-terminal"></span><span>Oil and LNG export terminals</span></div>
            <div class="hormuz-legend-row"><span class="hormuz-legend-dot hormuz-legend-dot-tanker"></span><span>Tanker traffic along shipping lanes</span></div>
            <div class="hormuz-legend-row"><span class="hormuz-legend-line hormuz-legend-line-choke"></span><span>Hormuz transit gauntlet and range rings</span></div>
          </div>
        </aside>
      </div>`;
    }

    function sampleRouteWithBearing(coords, steps) {
      const segments = [];
      let total = 0;
      for (let i = 1; i < coords.length; i += 1) {
        ...
        segments.push({ a, b, len, bearing });
        total += len;
      }
      const out = [];
      if (!segments.length || total <= 0) return out;
      for (let i = 0; i < steps; i += 1) {
        const t = (i + 0.5) / steps;
        let target = t * total;
        for (let s = 0; s < segments.length; s += 1) {
          const seg = segments[s];
          if (target <= seg.len || s === segments.length - 1) {
            const u = Math.max(0, Math.min(1, target / seg.len));
            out.push({
              lon: seg.a[0] + u * (seg.b[0] - seg.a[0]),
              lat: seg.a[1] + u * (seg.b[1] - seg.a[1]),
              bearing: seg.bearing,
            });
            break;
          }
    ...
    function hormuzEnergyData() {
      const fallbackLand = [
        {
          type: "Feature",
          properties: { side: "iran-island", name: "Qeshm Island" },
          geometry: {
            type: "Polygon",
            coordinates: [[
              [55.18, 26.66], [55.42, 26.78], [55.92, 26.82], [56.12, 26.72],
              [56.05, 26.62], [55.75, 26.58], [55.42, 26.55], [55.18, 26.60], [55.18, 26.66],
            ]],
          },
        },
        {
          type: "Feature",
          properties: { side: "iran-island", name: "Hormuz Island" },
          geometry: {
            type: "Polygon",
            coordinates: [[
              [56.42, 27.04], [56.50, 27.10], [56.55, 27.05], [56.50, 26.98], [56.42, 27.04],
            ]],
          },
        },
        {
          type: "Feature",
          properties: { side: "iran-island", name: "Kish Island" },
          geometry: {
            type: "Polygon",
            coordinates: [[
              [53.90, 26.50], [54.05, 26.58], [54.10, 26.54], [54.05, 26.49], [53.92, 26.47], [53.90, 26.50],
            ]],
          },
        },
      ];
      ...
      const pipelines = [
        {
          type: "Feature",
          properties: { name: "ADCOP", capacity: "1.5M bpd", operator: "ADNOC" },
          geometry: {
            type: "LineString",
            coordinates: [[54.05, 23.85], [54.55, 24.05], [55.10, 24.30], [55.55, 24.55], [55.95, 24.78], [56.25, 24.96], [56.34, 25.13]],
          },
        },
        {
          type: "Feature",
          properties: { name: "Goreh-Jask", capacity: "300K bpd", operator: "NIOC" },
          geometry: {
            type: "LineString",
            coordinates: [[54.85, 28.40], [55.30, 28.05], [55.75, 27.55], [56.20, 27.05], [56.70, 26.50], [57.20, 26.05], [57.55, 25.85], [57.77, 25.66]],
          },
        },
        {
          type: "Feature",
          properties: { name: "Saudi Petroline (E-W)", capacity: "5M bpd", operator: "Aramco" },
          geometry: {
            type: "LineString",
            coordinates: [[53.50, 25.55], [53.05, 25.42], [52.45, 25.25]],
          },
        },
      ];

      const terminals = [
        { label: "Bandar Abbas", lon: 56.27, lat: 27.18, role: "terminal", weight: 92, country: "Iran", dx: 8, dy: -6 },
        { label: "Kish Island", lon: 53.97, lat: 26.55, role: "terminal", weight: 58, country: "Iran", dx: -10, dy: 12 },
        { label: "Sirri Island", lon: 54.53, lat: 25.90, role: "terminal", weight: 54, country: "Iran", dx: 10, dy: -8 },
        { label: "Jask terminal", lon: 57.77, lat: 25.66, role: "terminal", weight: 70, country: "Iran", dx: 10, dy: -6 },
        { label: "Fujairah", lon: 56.34, lat: 25.13, role: "terminal", weight: 88, country: "UAE", dx: 12, dy: 12 },
        { label: "Sohar", lon: 56.71, lat: 24.36, role: "terminal", weight: 64, country: "Oman", dx: 12, dy: 8 },
        { label: "Habshan (inland)", lon: 54.05, lat: 23.85, role: "origin", weight: 68, country: "UAE", dx: -14, dy: 10 },
        { label: "Khasab", lon: 56.25, lat: 26.18, role: "port", weight: 50, country: "Oman", dx: 10, dy: 12 },
        { label: "Qeshm port", lon: 55.78, lat: 26.78, role: "port", weight: 46, country: "Iran", dx: -36, dy: -8 },
        { label: "Hormuz Island", lon: 56.46, lat: 27.06, role: "port", weight: 44, country: "Iran", dx: 10, dy: -8 },
      ];

      const outboundCoords = [[54.20, 26.22], [54.86, 26.22], [55.34, 26.25], [55.88, 26.24], [56.24, 26.18], [56.46, 26.10], [56.78, 25.98], [57.20, 25.84], [57.78, 25.69], [58.30, 25.50]];
      const inboundCoords = [[58.30, 25.30], [57.78, 25.50], [57.20, 25.72], [56.78, 25.86], [56.46, 25.96], [56.25, 26.03], [55.88, 26.08], [55.35, 26.08], [54.86, 26.05], [54.20, 26.05]];
      const fujairahCoords = [[56.34, 25.13], [56.48, 25.46], [56.62, 25.72], [56.78, 25.86]];

      const tankers = [
        ...sampleRouteWithBearing(outboundCoords, 28).map((p, idx) => ({ ...p, idx, lane: "out" })),
        ...sampleRouteWithBearing(inboundCoords, 28).map((p, idx) => ({ ...p, idx, lane: "in" })),
        ...sampleRouteWithBearing(fujairahCoords, 14).map((p, idx) => ({ ...p, idx, lane: "fuj" })),
      ];

      const lanes = [
        { type: "Feature", properties: { name: "Outbound shipping lane", kind: "shipping" }, geometry: { type: "LineString", coordinates: outboundCoords } },
        { type: "Feature", properties: { name: "Inbound shipping lane", kind: "shipping" }, geometry: { type: "LineString", coordinates: inboundCoords } },
        { type: "Feature", properties: { name: "Fujairah approach", kind: "approach" }, geometry: { type: "LineString", coordinates: fujairahCoords } },
      ];

      const chokepoint = [{
        type: "Feature",
        properties: { name: "Hormuz chokepoint transect" },
        geometry: { type: "LineString", coordinates: [[56.30, 26.78], [56.46, 26.10]] },
      }];

      const rangeRingSpec = [
        { radiusLon: 0.55, radiusLat: 0.48, label: "30 nm" },
        { radiusLon: 1.05, radiusLat: 0.92, label: "60 nm" },
        { radiusLon: 1.75, radiusLat: 1.52, label: "100 nm" },
      ];
      const ringCenter = [56.38, 26.42];
      const rangeRings = rangeRingSpec.map((ring, ringIdx) => {
        const ringCoords = [];
        const steps = 96;
        for (let i = 0; i <= steps; i += 1) {
          const theta = (i / steps) * Math.PI * 2;
          ringCoords.push([
            ringCenter[0] + Math.cos(theta) * ring.radiusLon,
            ringCenter[1] + Math.sin(theta) * ring.radiusLat,
          ]);
        }
        return {
          type: "Feature",
          properties: { label: ring.label, ringIdx },
          geometry: { type: "LineString", coordinates: ringCoords },
        };
      });

      const annotations = [
        { label: "Persian Gulf", lon: 54.30, lat: 26.80, tone: "water", size: 16, dx: 0, dy: 0, angle: 0 },
        { label: "Gulf of Oman", lon: 58.05, lat: 25.10, tone: "water", size: 16, dx: 0, dy: 0, angle: -10 },
        { label: "STRAIT OF HORMUZ", lon: 56.50, lat: 26.42, tone: "choke", size: 13, dx: 24, dy: -16, angle: -22 },
        { label: "Iran", lon: 54.80, lat: 27.95, tone: "land", size: 18, dx: 0, dy: 0, angle: 0 },
        { label: "Oman", lon: 56.95, lat: 23.95, tone: "land", size: 18, dx: 0, dy: 0, angle: 0 },
        { label: "UAE", lon: 54.50, lat: 24.30, tone: "land", size: 17, dx: 0, dy: 0, angle: 0 },
      ];

      return { fallbackLand, pipelines, terminals, lanes, chokepoint, rangeRings, annotations, tankers };
    }

    function buildHormuzEnergySpec(width, height) {
      const accent = cssToken("--accent", "#00e87b");
      const accent2 = cssToken("--accent2", "#7de8ff");
      const accent3 = cssToken("--accent3", "#ffbf5f");
      const danger = cssToken("--danger", "#ff625f");
      const text = cssToken("--text", "#f3f6f8");
      const data = hormuzEnergyData();
      const centerLon = 55.95;
      const centerLat = 25.85;
      const lonSpan = 5.0;
      const latSpan = 5.5;
      const lonRad = lonSpan * Math.PI / 180;
      const latRad = latSpan * Math.PI / 180;
      const projectionScale = Math.max(2200, Math.min(width / lonRad, height / latRad));

      return {
        $schema: "https://vega.github.io/schema/vega/v6.json",
        description: "Persian Gulf energy corridor: pipelines, oil terminals, animated tanker traffic, and Hormuz chokepoint range rings.",
        background: "transparent",
        width,
        height,
        padding: 0,
        autosize: "none",
        signals: [
          { name: "pulse", value: 0, on: [{ events: { type: "timer", throttle: 90 }, update: "(pulse + 1) % 1200" }] },
          { name: "ringPulse", value: 0, on: [{ events: { type: "timer", throttle: 120 }, update: "(ringPulse + 1) % 360" }] },
        ],
        data: [
          {
            name: "countries",
          ...
          { name: "fallbackLand", values: data.fallbackLand },
          { name: "graticule", transform: [{ type: "graticule", extent: [[centerLon - lonSpan / 2, centerLat - latSpan / 2], [centerLon + lonSpan / 2, centerLat + latSpan / 2]], step: [1, 1] }] },
          { name: "pipelines", values: data.pipelines },
          { name: "lanes", values: data.lanes },
          { name: "chokepoint", values: data.chokepoint },
          {
            name: "terminals",
            values: data.terminals,
            transform: [{ type: "geopoint", projection: "gulf", fields: ["lon", "lat"], as: ["x", "y"] }],
          },
          {
            name: "annotations",
          ...
          {
            name: "pipelineLabels",
            values: data.pipelines.map((feature) => {
              const coords = feature.geometry.coordinates;
              const mid = coords[Math.floor(coords.length / 2)];
              return { name: feature.properties.name, capacity: feature.properties.capacity, lon: mid[0], lat: mid[1] };
            }),
            transform: [{ type: "geopoint", projection: "gulf", fields: ["lon", "lat"], as: ["x", "y"] }],
          },
          { name: "rangeRings", values: data.rangeRings },
        ],
        projections: [{
          name: "gulf",
          type: "mercator",
        ...
        scales: [
          { name: "terminalColor", type: "ordinal", domain: ["terminal", "origin", "port"], range: [accent3, accent, accent2] },
          { name: "terminalSize", type: "linear", domain: [40, 95], range: [70, 320] },
          { name: "pipelineColor", type: "ordinal", domain: ["ADNOC", "NIOC", "Aramco"], range: [accent, accent3, accent2] },
        ],
        marks: [
          {
            type: "rect",
          ...
          },
          {
            ...
            from: { data: "graticule" },
            interactive: false,
            transform: [{ type: "geoshape", projection: "gulf" }],
            encode: {
              enter: {
                fill: { value: null },
                stroke: { value: "rgba(255,255,255,0.06)" },
                strokeWidth: { value: 1 },
              },
            },
          },
          {
            type: "shape",
            from: { data: "fallbackLand" },
            interactive: false,
            transform: [{ type: "geoshape", projection: "gulf" }],
            encode: {
              update: {
                fill: [
                  { test: "datum.properties.side === 'iran'", value: "rgba(255,191,95,0.12)" },
                  { test: "datum.properties.side === 'iran-island'", value: "rgba(0,232,123,0.18)" },
                  { test: "datum.properties.side === 'uae'", value: "rgba(125,232,255,0.10)" },
                  { value: "rgba(125,232,255,0.10)" },
                ],
                stroke: { value: "rgba(243,246,248,0.10)" },
          ...
          },
          {
            ...
            encode: {
              update: {
                fill: [
                  { test: "datum.properties.name === 'Iran'", value: "rgba(255,191,95,0.30)" },
                  { test: "datum.properties.name === 'Saudi Arabia'", value: "rgba(125,232,255,0.18)" },
                  { test: "datum.properties.name === 'United Arab Emirates'", value: "rgba(125,232,255,0.22)" },
                  { test: "datum.properties.name === 'Oman'", value: "rgba(125,232,255,0.26)" },
                  { test: "datum.properties.name === 'Qatar'", value: "rgba(125,232,255,0.20)" },
                  { test: "datum.properties.name === 'Bahrain'", value: "rgba(125,232,255,0.20)" },
                  { test: "datum.properties.name === 'Kuwait'", value: "rgba(125,232,255,0.18)" },
                  { test: "datum.properties.name === 'Iraq'", value: "rgba(125,232,255,0.14)" },
                  { value: "rgba(125,232,255,0.10)" },
                ],
                stroke: { value: "rgba(243,246,248,0.38)" },
          ...
          },
          {
            ...
            encode: {
              update: {
                fill: { value: null },
                stroke: { value: danger },
                strokeOpacity: { signal: "0.18 + (sin((ringPulse + datum.properties.ringIdx * 120) * PI / 180) + 1) * 0.10" },
                strokeWidth: { value: 1.2 },
                strokeDash: { value: [3, 6] },
              },
            },
          },
          {
            ...
            encode: {
              update: {
                ...
                strokeWidth: [
                  { test: "datum.properties.kind === 'approach'", value: 1.6 },
                  { value: 2.2 },
                ],
                strokeOpacity: { value: 0.78 },
                strokeDash: { value: [10, 8] },
                strokeDashOffset: { signal: "-pulse" },
              },
            },
          },
          {
            ...
            encode: {
              update: {
                fill: { value: null },
                stroke: { scale: "pipelineColor", field: "properties.operator" },
                strokeWidth: { value: 2.6 },
                strokeOpacity: { value: 0.92 },
                strokeDash: { value: [2, 5] },
                strokeDashOffset: { signal: "pulse * 1.6" },
              },
              hover: {
                strokeWidth: { value: 3.6 },
          ...
          },
          {
            ...
            encode: {
              enter: {
                ...
                size: { value: 44 },
                fill: [
                  { test: "datum.lane === 'fuj'", value: accent2 },
                  { test: "datum.lane === 'in'", value: accent },
                  { value: accent3 },
                ],
                stroke: { value: "rgba(255,255,255,0.85)" },
                strokeWidth: { value: 0.6 },
              },
              update: {
                angle: { field: "bearing" },
                opacity: { signal: "(((pulse * 1.5) + datum.idx * 5) % 100) < 22 ? 1 : 0.12" },
                size: { signal: "(((pulse * 1.5) + datum.idx * 5) % 100) < 22 ? 64 : 36" },
              },
            },
          },
          {
            ...
            encode: {
              enter: {
                x: { field: "x" }, y: { field: "y" },
                shape: [
                  { test: "datum.role === 'terminal'", value: "square" },
                  { test: "datum.role === 'origin'", value: "diamond" },
                  { value: "circle" },
                ],
                size: { scale: "terminalSize", field: "weight" },
                fill: { scale: "terminalColor", field: "role" },
              ...
              hover: {
                size: { signal: "scale('terminalSize', datum.weight) * 1.45" },
                stroke: { value: text },
              },
            },
          },
          {
            ...
            encode: {
              update: {
                ...
                size: { signal: "scale('terminalSize', datum.weight) * (1.7 + (pulse % 24) / 60)" },
                fill: { value: null },
                stroke: { scale: "terminalColor", field: "role" },
                strokeOpacity: { signal: "0.46 - ((pulse % 24) / 80)" },
                strokeWidth: { value: 1 },
              },
            },
          },
          {
            ...
            encode: {
              enter: {
                ...
                text: { field: "label" },
                fill: { value: "rgba(243,246,248,0.86)" },
                font: { value: "SFMono-Regular, Consolas, monospace" },
                fontSize: { value: 10 },
                fontWeight: { value: 700 },
                limit: { value: 130 },
          ...
          {
            type: "text",
            from: { data: "pipelineLabels" },
            interactive: false,
            encode: {
              enter: {
                x: { field: "x" }, y: { field: "y" },
                dy: { value: -8 },
                text: { signal: "datum.name + ' - ' + datum.capacity" },
                align: { value: "center" },
          ...
          },
          {
            ...
            encode: {
              enter: {
                ...
                fill: [
                  { test: "datum.tone === 'choke'", value: danger },
                  { test: "datum.tone === 'water'", value: "rgba(125,232,255,0.62)" },
                  { value: "rgba(243,246,248,0.74)" },
                ],
                font: { value: "SFMono-Regular, Consolas, monospace" },
                fontSize: { field: "size" },
                fontWeight: { value: 760 },
    ...
    function renderHormuzEnergyMap(attempt = 0) {
      const node = $("hormuz-vega-map-2");
      if (!node) return;
      if (typeof window.vega?.View !== "function" || typeof window.vega?.parse !== "function") {
        if (attempt < 30) {
          window.setTimeout(() => renderHormuzEnergyMap(attempt + 1), 120);
        } else {
          node.innerHTML = `<div class="hormuz-map-fallback">Vega renderer unavailable. Energy corridor map could not load.</div>`;
        }
        return;
      }
      const rect = node.getBoundingClientRect();
      const width = Math.max(320, Math.floor(rect.width || node.clientWidth || 720));
      const height = Math.max(260, Math.floor(rect.height || node.clientHeight || 460));
      const renderKey = `${width}x${height}`;
      if (node.dataset.renderKey === renderKey && node.__hormuzEnergyView) return;
      if (node.__hormuzEnergyView && typeof node.__hormuzEnergyView.finalize === "function") {
        node.__hormuzEnergyView.finalize();
      }
      node.dataset.renderKey = renderKey;
      node.innerHTML = "";
      try {
        const spec = buildHormuzEnergySpec(width, height);
        const view = new window.vega.View(window.vega.parse(spec), {
          renderer: "canvas",
          container: node,
          hover: true,
        });
        node.__hormuzEnergyView = view;
        view.runAsync().then(() => {
          animate(node, { opacity: [0.72, 1], scale: [0.985, 1], duration: 560, ease: "out(3)" });
        }).catch(() => {
          node.innerHTML = `<div class="hormuz-map-fallback">Vega energy map render failed. Check the local console for details.</div>`;
        });
      } catch (err) {
        node.innerHTML = `<div class="hormuz-map-fallback">Vega energy map render failed. Check the local console for details.</div>`;
      }
    ...
    function renderHormuzVegaMap(attempt = 0) {
      const node = $("hormuz-vega-map");
      if (!node) return;
      if (typeof window.vega?.View !== "function" || typeof window.vega?.parse !== "function") {
        if (attempt < 30) {
          window.setTimeout(() => renderHormuzVegaMap(attempt + 1), 120);
        } else {
          node.innerHTML = `<div class="hormuz-map-fallback">Vega renderer unavailable. Strait of Hormuz map layer could not load.</div>`;
        }
        return;
      }
      const rect = node.getBoundingClientRect();
      const width = Math.max(300, Math.floor(rect.width || node.clientWidth || 640));
      const height = Math.max(240, Math.floor(rect.height || node.clientHeight || 420));
      const renderKey = `${width}x${height}`;
      if (node.dataset.renderKey === renderKey && node.__hormuzView) return;
      if (node.__hormuzView && typeof node.__hormuzView.finalize === "function") {
        node.__hormuzView.finalize();
      }
      node.dataset.renderKey = renderKey;
      node.innerHTML = "";
      try {
        const spec = buildHormuzVegaSpec(width, height);
        const view = new window.vega.View(window.vega.parse(spec), {
          renderer: "canvas",
          container: node,
          hover: true,
        });
        node.__hormuzView = view;
        view.runAsync().then(() => {
          animate(node, { opacity: [0.76, 1], scale: [0.99, 1], duration: 520, ease: "out(3)" });
        }).catch(() => {
          node.innerHTML = `<div class="hormuz-map-fallback">Vega map render failed. Check the local console for details.</div>`;
        });
      } catch (err) {
        node.innerHTML = `<div class="hormuz-map-fallback">Vega map render failed. Check the local console for details.</div>`;
      }
    ...
    function renderGeoSignal() {
      if (dashboardId === "iran") return renderIranSceneWrapper();
    ...
    let hormuzResizeTimer = 0;
    window.addEventListener("resize", () => {
      if (dashboardId !== "iran") return;
      window.clearTimeout(hormuzResizeTimer);
      hormuzResizeTimer = window.setTimeout(() => {
        const node1 = $("hormuz-vega-map");
        if (node1) node1.dataset.renderKey = "";
        const node2 = $("hormuz-vega-map-2");
        if (node2) node2.dataset.renderKey = "";
        renderIranActiveSlideMap();
      }, 160);
    });
