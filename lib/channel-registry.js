...
const PROVIDERS = {
  ...
  "katechon-platform": {
    ...
    label: "Katechon Platform Context",
    auth: "none",
    realtime: "server-side Katechon technology summary for the channel runtime, Kat, surfaces, and share graph",
    docsUrl: "/docs/channel-apis.md",
    capabilities: ["snapshot", "events", "rankings", "entity_detail", "relationships", "search"],
    routes: ["GET /api/channels/katechon-technology/live"],
  },
  "channel-synthetic": {
...
const CHANNEL_API_IDEAS = {
  ...
  "katechon-technology": [
    { id: "katechon-platform", use: "Katechon technology summary and channel runtime context", fit: "platform architecture, runtime loop, content seeds, and share/fork state graph" },
...
function chartBindingsForChannel(channel) {
  ...
  for (const provider of [channel.liveProvider, ...(channel.providers || [])]) {
    ...
    else if (provider === "katechon-platform") add("liveData.technologyStack", "liveData.runtimeLoop", "liveData.contentSeeds");
  }
...
const CHANNEL_OVERRIDES = {
  ...
  "katechon-technology": {
    category: "platform",
    liveProvider: "katechon-platform",
    providers: ["katechon-platform"],
    contract: "katechon-technology-v1",
    status: "public-adapter",
    summary: "Katechon Technology is a first-class platform channel summarizing the channel runtime, Kat continuity layer, specialist agents, mutable surfaces, and share/fork state graph.",
