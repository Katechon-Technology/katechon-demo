...
function dataCapabilitiesForChannel(channel) {
  ...
  if (["hyperliquid", "eia-grid"].includes(channel.liveProvider)) {
    ids.add("timeseries");
    ids.add("historical_state");
  }
  if (channel.liveProvider === "polymarket" || channel.liveProvider === "pumpfun") ids.add("rankings");
...
function channelGenerationSpec(channel) {
  ...
  return {
    ...
    availableViews: Array.from(new Set(["default", "overview", "deep_dive", "comparison", "event_investigation", "historical_replay", "risk_anomaly", "relationship_map", ...hints.preferredViews])).map((id) => ({
      id,
      ...VIEW_PRESETS[id],
    })).filter((view) => view.label),
    layouts: Array.from(new Set(["overview", "deep_dive", "comparison", "event_investigation", "historical_replay", "risk_anomaly", "relationship_map", ...hints.preferredViews])).map((id) => ({
      id,
      ...VIEW_PRESETS[id],
    ...
    provenance: {
      ...
      syntheticPolicy: "Synthetic fallback must stay visible in source chips and narration; never label it as live_api.",
    },
    updateContract: {
...
function channelDocs(channel) {
  ...
  return {
    ...
    katContract: [
      ...
      "Attach provenance to generated charts, insights, tables, metrics, timelines, and actions.",
