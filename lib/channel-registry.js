...
const PROVIDERS = {
  ...
  "local-deck-json": {
    id: "local-deck-json",
    label: "Local Katechon Technology Context",
    auth: "none",
    realtime: "server-side local Katechon technology summary plus generated deck state",
    docsUrl: "/decks/dune/deck.json",
    capabilities: ["snapshot", "events", "rankings", "entity_detail", "relationships", "search"],
    routes: ["GET public/decks/dune/deck.json through the channel runtime", "GET /api/channels/dune-deck/live"],
  },
  "channel-synthetic": {
...
const COMPONENT_REGISTRY = {
  ...
  "vega-chart": {
    ...
    bindings: [
      ...
      "liveData.fuelMix",
      "liveData.corridors",
      "liveData.technologyStack",
      "liveData.runtimeLoop",
      "liveData.contentSeeds",
      "liveData.slides",
      "liveSummary.metrics",
      "liveSummary.feed",
      "dashboard.metrics",
...
const CATEGORY_GENERATION_HINTS = {
  ...
  platform: {
    audience: "builders, investors, and product operators",
    preferredViews: ["overview", "briefing", "investor", "relationship_map"],
    preferredComponents: ["metric-strip", "insight-card", "relationship-graph", "event-timeline", "action-panel"],
  },
};
...
const CHANNEL_API_IDEAS = {
  ...
  "dune-deck": [
    { id: "local-deck-json", use: "Katechon technology summary and generated deck state", fit: "platform architecture, runtime loop, content seeds, and generated slide history" },
    { id: "product-analytics", use: "viewer/channel engagement", fit: "share, replay, fork, and content-generation proof" },
  ],
};
...
function chartBindingsForChannel(channel) {
  ...
  for (const provider of [channel.liveProvider, ...(channel.providers || [])]) {
    ...
    else if (provider === "local-deck-json") add("liveData.technologyStack", "liveData.runtimeLoop", "liveData.contentSeeds", "liveData.slides");
  }
...
const CHANNEL_OVERRIDES = {
  ...
  "dune-deck": {
    ...
    contract: "katechon-technology-v1",
    status: "public-adapter-local",
    summary: "Katechon Technology is a first-class platform channel summarizing the channel runtime, Kat continuity layer, specialist agents, mutable surfaces, and share/fork state graph, with the former deck acting as local source material for generated content.",
...
function wordsForCategory(category) {
  const byCategory = {
    ...
    platform: { primary: "Channels", secondary: "Runtime", feedPrimary: "platform", feedSecondary: "Surface" },
