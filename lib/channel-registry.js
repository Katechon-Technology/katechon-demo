...
function chartBindingsForChannel(channel) {
  ...
  if (["pumpfun", "dexscreener", "coingecko-pumpfun"].includes(channel.liveProvider)) return ["liveData.tokens", "liveSummary.metrics", "liveSummary.feed"];
...
const CHANNEL_OVERRIDES = {
  ...
  arena: {
    category: "ai-eval",
    liveProvider: "github-actions",
    providers: ["github-actions"],
    contract: "agent-match-v1",
    status: "public-adapter-optional",
    summary: "AI Arena reads public GitHub Actions workflow runs when ARENA_GITHUB_REPO is configured; otherwise it returns an explicit unavailable public-source state.",
  },
  glance: {
  ...
  "meme-coin": {
    ...
    liveProvider: "dexscreener",
    providers: ["dexscreener", "coingecko-pumpfun"],
    contract: "token-velocity-v1",
    status: "live",
    summary: "Meme Coin turns public DEX Screener token, boost, pair, price-change, volume, and liquidity data into shareable attention, liquidity-risk, fragility, and narrative-decay boards.",
    commandPlaceholder: "Ask for velocity, liquidity risk, fragility, or narrative decay...",
    heroPrompts: [
